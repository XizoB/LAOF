...
def main(cfg: DictConfig):    
    ...
    # 创建IDM/WM模型
    idm, wm, fm = utils.create_dynamics_continue_models_flow(cfg.model)
    ...
    def test_step():
        ...
        _, eval_metrics = utils.eval_latent_res_continue(cfg.model, eval_data, idm)

        logger(step, wm_loss_test=wm_loss, fm_loss_test=fm_loss, global_step=step * cfg.stage1.bs, **eval_metrics)
    ...
    for step in loop(cfg.stage1.steps + 1, desc="[green bold](stage-1) Training IDM + FDM with TFRecord"):
        ...
        if step > 0 and (step % 5_000 == 0 or step == cfg.stage1.steps):
            torch.save(
                dict(
                    **doy.get_state_dicts(wm=wm, idm=idm, fm=fm, opt=opt),
                    step=step,
                    cfg=cfg,
                    logger=logger,
                ),
