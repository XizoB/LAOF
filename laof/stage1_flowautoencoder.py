...
def main(cfg: DictConfig):    
    ...
    # 创建IDM/WM模型
    idm, _, fm = utils.create_dynamics_flowautoencoder(cfg.model)
    ...
    def train_step():
        ...
        vq_loss, vq_perp = idm.label_onehorizon_autoflow(batch)
        fm_loss = fm.label_flow(batch)
        loss = vq_loss + fm_loss


        opt.zero_grad()
        loss.backward()
        grad_norm = torch.nn.utils.clip_grad_norm_([*idm.parameters(), *fm.parameters()], 2)
        opt.step()
    ...
    def test_step():
        ...
        _, eval_metrics = utils.eval_latent_autoflow(cfg.model, eval_data, idm)

        logger(step, fm_loss_test=fm_loss, global_step=step * cfg.stage1.bs, **eval_metrics)
    ...
    for step in loop(cfg.stage1.steps + 1, desc="[green bold](stage-1) Training IDM + FDM with TFRecord"):
        ...
        if step > 0 and (step % 5_000 == 0 or step == cfg.stage1.steps):
            torch.save(
                dict(
                    **doy.get_state_dicts(idm=idm, fm=fm, opt=opt),
                    step=step,
                    cfg=cfg,
                    logger=logger,
                ),
