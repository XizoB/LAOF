...
def main(cfg: DictConfig):    
    ...
    # 创建IDM/WM模型
    idm, wm, fm = utils.create_dynamics_models_flow(cfg.model)
    ...
    def train_step():
        ...
        vq_loss, vq_perp = idm.label_onehorizon_res(batch)
        wm_loss = wm.label_onehorizon(batch)
        fm_loss = fm.label_flow(batch)
        loss = wm_loss + vq_loss + fm_loss
    ...
    def test_step():
        ...
        _, eval_metrics = utils.eval_latent_res(cfg.model, eval_data, idm)
