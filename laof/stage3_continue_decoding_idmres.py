...
from utils.utils import create_decoder
...
def main(cfg: DictConfig):    
    ...
    # 导入IDM/WM 模型参数
    state_dict = torch.load(paths.get_latent_policy_path(config.get(file_cfg=cfg).exp_name), weights_only=False)

    # 导入配置参数
    cfg = config.get(file_cfg=cfg, base_cfg=state_dict["cfg"], reload_keys=["stage3"])
    ...
    idm, _ = utils.create_dynamics_continue_models(cfg.model, state_dicts=torch.load(models_path, weights_only=False))
    idm.eval()
    ...
    # run, logger = config.wandb_init("laof_stage3", config.get_wandb_cfg(cfg), wandb_enabled=False)
    run, logger = config.wandb_init("laof_stage3", config.get_wandb_cfg(cfg), wandb_enabled=True)
    ...
    def action_selection_hook(next_obs: torch.Tensor, global_step: int = None, action=None):
        # sample action
        hidden_base = policy.conv_stack(next_obs) # torch.Size([64, 3, 64, 64]) -> torch.Size([64, 8192])

        hidden_rl = F.relu(policy.fc_rl(hidden_base)) # torch.Size([64, 8192]) -> torch.Size([512, 256])
        hidden_sl = F.relu(policy.fc_sl(hidden_base))

        logits_rl = policy.policy_head_rl(hidden_rl) # torch.Size([512, 256]) -> torch.Size([512, 15])
        ...
        if not action_given:
            ...
            if len(buf_obs) == 3:
                buf_la.append(idm.label_res(torch.cat(list(buf_obs), dim=1), training=False)[0]["la"])
                # buf_la.append(idm(torch.cat(list(buf_obs), dim=1)[:, -2:])[0]["la"])
