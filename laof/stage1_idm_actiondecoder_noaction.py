...
def main(cfg: DictConfig):    
    ...
    # 创建IDM/WM模型
    idm, wm, _, ad = utils.create_dynamics_models_flow_decode(cfg.model)
    ...
    def train_step():
        ...
        loss = ratio * (wm_loss + vq_loss + ad_loss)
    ...
    def test_step():
        ...
        logger(step, wm_loss_test=wm_loss, ad_loss_test=ad_loss, global_step=step * cfg.stage1.bs, **eval_metrics)
    ...
    for step in loop(cfg.stage1.steps + 1, desc="[green bold](stage-1) Training IDM + FDM with TFRecord"):
        ...
        if step > 0 and (step % 5_000 == 0 or step == cfg.stage1.steps):
            torch.save(
                dict(
                    **doy.get_state_dicts(wm=wm, idm=idm, ad=ad, opt=opt),
                    step=step,
                    cfg=cfg,
                    logger=logger,
                ),
