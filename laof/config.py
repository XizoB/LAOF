...
@dataclass
class Stage2IRLConfig:
    """Hyperparams for inverse reinforcement learning the latent policy (from the latent IDM)"""
...
class Config:
    ...
    stage1: Stage1Config
    stage2: Stage2Config
    stage2irl: Stage2IRLConfig
    stage3: Stage3Config


def get(
    file_cfg: DictConfig | None = None,
    ...
    file_cfg = OmegaConf.create(copy.deepcopy(OmegaConf.to_container(file_cfg, resolve=False)))
...
def wandb_init(
    ...
    run_name = config["exp_name"].split('/')[-1]
