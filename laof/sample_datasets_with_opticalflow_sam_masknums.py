...
def flow_to_rgb(flow, method="opencv", sigma=0.01):
    ...
    if method == "opencv":
        ...
        hsv_float = np.stack([H * 180 / 255, S, V], axis=-1)
...
def process_optical_flow(obs_raw_sequence, raft_model, flow_viz_method="opencv", sigma=0.01):
    """
    处理光流数据，转换为RGB图像后resize
    
    Args:
        obs_raw_sequence: numpy array (N, 512, 512, 3)
        raft_model: RAFT模型
        flow_viz_method: str - 光流可视化方法 ("opencv" 或 "paper")
    
    Returns:
        obs_of: numpy array (N, 64, 64, 3) - 光流RGB图像resize后的结果
        flow_512_rgb: numpy array (N, 512, 512, 3) - 原始512x512光流RGB图像
    """
    if len(obs_raw_sequence) < 2:
        # 如果只有一帧，直接返回零填充的结果
        obs_of = np.zeros((len(obs_raw_sequence), 64, 64, 3), dtype=np.uint8)
        flow_512_rgb = np.zeros((len(obs_raw_sequence), 512, 512, 3), dtype=np.uint8)
        return obs_of, flow_512_rgb
    ...
    if flow_results:
        ...
        for flow in flow_hwc:
            # 将光流转换为RGB图像 - 使用flow_viz函数替换flow_to_rgb
            ...
            # 使用OpenCV resize到64x64
            resized_rgb = cv2.resize(flow_rgb, (64, 64), interpolation=cv2.INTER_CUBIC)
            resized_flow_rgb.append(resized_rgb)
        
        obs_of = np.array(resized_flow_rgb)  # (N, 64, 64, 3)
        flow_512_rgb = np.array(flow_rgb_512_list)  # (N, 512, 512, 3)
    else:
        # 如果没有光流结果，返回零填充
        obs_of = np.zeros((len(obs_raw_sequence), 64, 64, 3), dtype=np.uint8)
        flow_512_rgb = np.zeros((len(obs_raw_sequence), 512, 512, 3), dtype=np.uint8)
...
def process_langsam(obs_raw_sequence, langsam_model, text_prompt="object"):
    ...
    Returns:
        ...
        mask_nums: numpy array (N,) - 每帧检测到的mask数量
    """
    ...
    for result in results:
        ...
        all_mask_nums.append(mask_num)
    
    # 转换为numpy array (N, 1, 512, 512)
    masks_array = np.array(all_masks)
    mask_nums = np.array(all_mask_nums, dtype=np.int32)  # (N,)
    ...
    return masks_array, mask_nums


def apply_mask_to_flow(flow_rgb_data, masks_array):
    """
    将mask应用到光流RGB图像上并resize
    ...
    Returns:
        obs_sam: numpy array (N, 64, 64, 3) - mask应用到光流RGB图像后resize的结果
    """
    # 应用mask到光流RGB图像
    ...
    for i in range(len(flow_rgb_data)):
        ...
        # 使用OpenCV resize到64x64
        masked_flow_rgb_resized = cv2.resize(masked_flow_rgb, (64, 64), interpolation=cv2.INTER_CUBIC)
        
        masked_flow_rgb_list.append(masked_flow_rgb_resized)
    
    # 转换为numpy array (N, 64, 64, 3)
...
def sample_expert_data(policy, env, raft_model, langsam_model, text_prompt, max_ep_steps=1000, max_ep_returns=10.0, nums_ep=50, nums_dataset=100, save_dir=None, flow_viz_method="opencv", sigma=0.01):
    ...
    while dataset_count < nums_dataset:
        ...
        # 存储当前步骤数据
        current_traj_obs.append(observe[0])  # (64, 64, 3)
        current_traj_ta.append(action.cpu().numpy()[0])  # scalar
        current_traj_obs_raw.append(obs_raw)  # (512, 512, 3)
        ...
        if done[0] or current_ep_steps >= max_ep_steps:
            ...
            if current_ep_return >= max_ep_returns and len(current_traj_obs) >= 3:
                ...
                obs_of, flow_512_rgb = process_optical_flow(obs_raw_array, raft_model, flow_viz_method=flow_viz_method, sigma=sigma)  # (steps, 64, 64, 3), (steps, 512, 512, 3)

                print(f"  处理LangSAM数据... ")
                masks_array, mask_nums = process_langsam(obs_raw_array, langsam_model, text_prompt=text_prompt)  # (steps, 1, 512, 512), (steps,)
                
                print(f"  应用mask到光流RGB图像... ")
                obs_sam = apply_mask_to_flow(flow_512_rgb, masks_array)  # (steps, 64, 64, 3)
                
                # 创建单条轨迹的数据字典
                trajectory_data = {
                    'obs': np.array(current_traj_obs, dtype=np.uint8),      # (steps, 64, 64, 3)
                    ...
                    'obs_of': obs_of.astype(np.uint8),                      # (steps, 64, 64, 3) - 光流RGB图像
                    'obs_sam': obs_sam.astype(np.uint8),                    # (steps, 64, 64, 3) - masked光流RGB图像
                    'mask_nums':  np.array(mask_nums, dtype=np.int8),          # (steps,) - 每帧检测到的mask数量
                }
...
def save_hdf5_dataset(trajectories_list, save_dir, dataset_idx):
    ...
    with h5py.File(save_path, 'w') as f:
        for i, traj_data in enumerate(trajectories_list):
            ...
            demo_group.create_dataset('mask_nums', data=traj_data['mask_nums'], compression='gzip')
    ...
    if trajectories_list:
        ...
        print(f"  示例轨迹数据形状:")
        print(f"    obs.shape: {first_traj['obs'].shape} {type(first_traj['obs'])}")
        ...
        print(f"    obs_of.shape: {first_traj['obs_of'].shape} {type(first_traj['obs_of'])}")
        print(f"    obs_sam.shape: {first_traj['obs_sam'].shape} {type(first_traj['obs_sam'])} (masked光流RGB图像)")
        print(f"    mask_nums.shape: {first_traj['mask_nums'].shape} {type(first_traj['mask_nums'])} (每帧mask数量)")
...
if __name__ == "__main__":
    ...
    # 根据环境名称动态设置保存路径
    save_dir = f"/root/Docker_RoboLearn/Benchmark/LAPO_IRL/lapo/expert_data/opticalflow/{cfg.env_name}/train"
